...
<template>
  <div p6 flex="~ col gap-2" items-center max-h-screen h-screen overflow-auto relative>
    ...
    <p>{{ t('intro-1') }} <b text-teal-500>{{ t('intro-2') }}</b>。</p>
    <p>{{ t('intro-3') }}</p>
    <div h-1px w-10 border="b base" m4 />

    <Sentence :word="t('example-1')" :revealed="true" answer=" 门  " />
    <p>{{ t('intro-4') }} <b text-teal-600>{{ t('intro-5') }}</b> {{ t('intro-6') }}</p>

    <Sentence :word="t('example-2')" :revealed="true" answer="   仓" />
    <p>{{ t('intro-7') }} <b text-orange-400>{{ t('intro-8') }}</b> {{ t('intro-9') }}</p>

    <Sentence :word="t('example-3')" :revealed="true" answer="桥它拖 " />
    <p max-w-130>
      {{ t('intro-10') }} <b>{{ t('intro-11') }}</b> {{ t('intro-12') }}<br>
      {{ t('intro-13') }} <b op50>{{ t('intro-14') }}</b> {{ t('intro-15') }} <b op50>{{ t('intro-14') }}</b> {{ t('intro-16') }}<br>
      {{ t('intro-17') }} <b text-orange-400>{{ t('intro-18') }}</b> {{ t('intro-19') }}
    </p>

    <Sentence :word="t('example-4')" :revealed="true" answer="武运昌隆" />
    <p>{{ t('intro-20') }}</p>
