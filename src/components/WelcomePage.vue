...
<template>
  <div p6 flex="~ col gap-2" items-center max-h-screen h-screen overflow-auto relative>
    ...
    <p>你有十次的机会猜一<b text-teal-600>四字成语</b>。</p>
    <p>每次猜测后，拼音与汉字的颜色会指示你的猜测与正确答案的距离。</p>
    <div h-1px w-10 border="b base" m4 />

    <Sentence :word="'班门弄斧'" :revealed="true" :answer="parseWord(' 门  ')" />
    <p>第二个字 <b text-teal-600 font-serif text-xl>门</b> 为青色，表示其出现在答案中且在正确的位置。</p>

    <Sentence :word="'仓颉造字'" :revealed="true" :answer="parseWord('   仓')" />
    <p>第一个字 <b text-orange-400 font-serif text-xl>仓</b> 为橙色，表示其出现在答案中，但并不在第一个字。</p>

    <Sentence :word="'巧夺天工'" :revealed="true" :answer="parseWord('桥它拖 ')" />
    <p max-w-130>
      每个格子的 <b>字</b>、<b>声母</b>、<b>韵母</b>、<b>声调</b> 都会独立进行颜色的指示。<br>例如，第一个 <b font-serif text-xl>巧</b> 字面为灰色，但其声母韵与母均为青色，代表该位置的正确答案为其同音字但非 <b font-serif text-xl>巧</b> 字本身。<br>同理，第二个字中韵母 <span font-serif text-xl>uo</span> 为橙色，代表其韵母出现在四个字之中，但非位居第二。以此类推。
    </p>
