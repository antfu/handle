...
<template>
  <div p6 flex="~ col gap-2" items-center max-h-screen h-screen overflow-auto>
    ...
    <p>第二个字 <b text-teal-500>门</b> 为青色，表示其出现在答案中且在正确的位置。</p>

    <Sentence :word="'仓颉造字'" :revealed="true" :answer="parseWord('   仓')" />
    <p>第一个字 <b text-orange-400>仓</b> 为橙色，表示其出现在答案中，但并不在第一个字。</p>

    <Sentence :word="'巧夺天工'" :revealed="true" :answer="parseWord('桥它拖 ')" />
    <p max-w-130>
      每个格子的 <b>字</b>、<b>声母</b>、<b>韵母</b>、<b>声调</b> 都会独立进行颜色的指示。<br>例如，第一个 <b>巧</b> 字面为灰色，但其声母韵与母均为青色，代表该位置的正确答案为其同音字但非 <b>巧</b> 字本身。<br>同理，第二个字中韵母 <code>uo</code> 为橙色，代表其韵母出现在四个字之中，但非位居第二。以此类推。
    </p>
    ...
    <button btn text-xl p="x4 y2" @click="start()">
      开始游戏!
    </button>
